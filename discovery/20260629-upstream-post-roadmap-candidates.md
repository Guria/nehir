...
| `6bd0bf75` + `644d9115` Private-API capability diagnostic + fallback-firing instrumentation (+ tidy) | New `PrivateAPIHealthDiagnostics.swift` (477 lines) + `FallbackFiringRecorder` instrumenting where private-API (SkyLight) fallbacks fire; `644d9115` tidies the surface. | **Nehir-native diagnostics track.** Nehir already instruments some SkyLight surfaces (`SpaceTopology.swift`, the M4-S1/M4-S2 diagnostics), but the original verdict overgeneralised that coverage. **Groom 2026-07-28:** the key-window focus path discards both `SLPSPostEventRecordTo` statuses and `_SLPSSetFrontProcessWithOptions`, silently returns on `GetProcessForPID`, and the command palette duplicates the gap while returning success. Port the narrow failure-recording concept, not the 477-line subsystem. See [`20260728-private-api-focus-statuses-are-silently-discarded.md`](20260728-private-api-focus-statuses-are-silently-discarded.md). | **🟢 → 🟡** (scoped observability candidate) | S | `Sources/Nehir/Core/PrivateAPIs.swift`, `Sources/Nehir/Core/Controller/RuntimeDiagnosticsCoordinator.swift`, `Sources/Nehir/UI/CommandPalette/CommandPaletteController.swift` |
