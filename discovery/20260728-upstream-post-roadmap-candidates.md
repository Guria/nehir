...
| BarutSRB/OmniWM#505 / `a4b8611a` Chrome PWA focus | The issue lane guessed 🟢 "probable N/A, upstream-regression-specific — unverified". The AX lane read `PrivateAPIs.swift`. | **🔴 stands.** Nehir carries the pre-fix bytes regardless of upstream's regression history; the fix is to the shared AltTab-derived event record, not to upstream's 0.5.7 admission cluster. |
...
1. **🔴 `a4b8611a` SkyLight key-window event record (XS).** Highest confidence in the
   sweep: Nehir carries the byte-for-byte pre-fix record on its primary focus path
   ...
   of the `0x20..<0x30` NaN block. Add a byte-layout unit test.
